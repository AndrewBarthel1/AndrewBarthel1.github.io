...
function snakeDeath() {
    if (gameOver) {
        ...
        if (sd >= score) {
            ...
            ctx.font = "20px Arial";
            ctx.fillText("Press 'P' to Restart in normal mode", 442, 250);
            ctx.fillText("and 'L' to Restart in hard mode", 464, 280);
            return;
        }
        sd++;
        return;
...
function render() {
    ...
    // Draw score
    ctx.fillStyle = "#C04735";
    ctx.fillRect(0, 398, 1200, 440);
    ctx.fillStyle = "rgb(82 43 75)";
    ctx.font = "16px Arial";
    ctx.fillText("Score: " + score, 200, 415);
    if (hardMode)
        ctx.fillText("Hard Mode Highscore: " + highscoreh, 600, 415);
    else
        ctx.fillText("Normal Mode Highscore: " + highscoren, 600, 415);
}
// Start the game loop
